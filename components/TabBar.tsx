        ...
        {tabs.map((tab) => (
          <Tooltip key={tab.name} delayDuration={500}>
            <TooltipTrigger asChild>
              <div
                className={`flex items-center gap-2 px-4 py-2 border-r border-border cursor-pointer group min-w-[150px] relative transition-all duration-200 ${activeFile === tab.name
                    ? "bg-editor-tabActive text-foreground shadow-sm"
                    : "bg-editor-tabInactive text-white hover:bg-muted/40 hover:text-foreground"
                  }`}
                onClick={() => tab.external ? window.open(tab.path, '_blank') : router.push(tab.path)}
              >
                {activeFile === tab.name && (
                  <div className="absolute top-0 left-0 right-0 h-0.5 bg-primary" />
                )}
                <FileCode className={`w-4 h-4 transition-colors ${activeFile === tab.name ? "text-syntax-function" : "text-white"
                  }`} />
